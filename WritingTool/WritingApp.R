...
get_new_sentence = function(my_sentence = NULL,
  ...
  for(generation in 1:ngenerations){ 
    ...
    gamete_sentences = foreach(parent_sentence_index = 1:length(population_of_sentences), .combine = 'c') %dopar% { # For each parent sentence in the current population,
      ...
      parent_sentence = sentence_genealogy[[generation]][parent_sentence_index] # pick the parent senteence
...
ui <- fluidPage(
  # App inputs
  textInput("query", "Bring your query sentence:"),
  ...
  numericInput("ngenerations", "Number of generations:", value = 100),
...
server <- function(input, output, session) {
  ...
  output$samples = renderTable(result())
